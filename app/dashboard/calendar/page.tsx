...
export default async function CalendarPage() {
  ...
  return (
    <div className="space-y-5"><div><h1 className="text-3xl font-bold">Calendar</h1><p className="text-muted-foreground">Monthly view with mobile-friendly agenda list.</p></div>
      <div className="hidden gap-3 md:grid md:grid-cols-7">{days.map((day) => {
      ...
      <div className="space-y-2 md:hidden">{reminders.length ? reminders.map((r) => <Link key={r.id} href={`/dashboard/reminders/${r.id}`} className="flex items-center justify-between gap-3 rounded-md border bg-card p-3 text-sm shadow-sm"><span className="truncate font-medium">{r.title}</span><span className="shrink-0 text-xs text-muted-foreground">{format(new Date(r.due_at), "MMM d, h:mm a")}</span></Link>) : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">No upcoming tasks this month.</p>}</div>
    </div>
  );
}
